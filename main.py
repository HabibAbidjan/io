...
def start(message):
    ...
    if user_id not in user_balances:
        ...
        # Referal kod bor-yo'qligini tekshiramiz
        if len(args) > 1:
            try:
                ref_id = int(args[1])
                if ref_id != user_id:
                    # Foydalanuvchi referal orqali keldi, bonus bering
...
def mines_start_with_stake(message):
    try:
        ...
        bot.send_message(message.chat.id, "❌ Noto‘g‘ri stavka. Raqam kiriting.")
        return
...
def handle_cell_click(call):
    ...
    if index in state['mines']:
        state['active'] = False
        # Bombaga bosildi, stavka yo'qotiladi (allaqachon balansdan yechilgan)
        show_result(call.message.chat.id, user_id, lose=True)
    else:
        # Multiplikatorni hisoblaymiz, masalan, har ochilgan toza katak uchun +0.5
...
def handle_cashout(call):
    ...
    # Yutuqni hisoblash: stavka * multiplikator
